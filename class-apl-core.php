...
class APL_Core {
	...
	private function post_list_loop( $post_list_slug ) {
		...
		if ( $wp_query_class->have_posts() ) {
			// BEFORE.
			$output .= wp_kses( apply_filters( 'apl_core_loop_before', $apl_design->before ), apl_allowed_tags_before() );

			$output .= wp_kses( $this->display_preset_list_content( $wp_query_class, $post_list_slug, $apl_design ), apl_allowed_tags_content() );

			// AFTER.
			$output .= wp_kses( apply_filters( 'apl_core_loop_after', $apl_design->after ), apl_allowed_tags_after() );
		} else {
			// EMPTY.
			$apl_options = apl_options_load();

			if ( ! empty( $apl_design->empty ) ) {
				$output .= wp_kses( $apl_design->empty, apl_allowed_tags_before() );
			} elseif ( true === $apl_options['default_empty_enable'] && ! empty( $apl_options['default_empty_output'] ) ) {
				$output .= wp_kses( $apl_options['default_empty_output'], apl_allowed_tags_before() );
			}
		}// End if().
