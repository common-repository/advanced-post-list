...
class APL_Admin {
	...
	private function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin\' huh?', 'advanced-post-list' ), esc_html( APL_VERSION ) );
	}
	...
	public function draft_post_list( $post_id, $post ) {
		if ( ! wp_verify_nonce( $_REQUEST['apl_nonce'], 'apl_nonce' ) ) {
			wp_die();
		}
	...
	public function tinymce_extra_vars() {
		...
		<script type="text/javascript">
			var apl_tinyMCE = <?php echo wp_json_encode( $apl_tinymce ); ?>;
		</script>
		<?php
	}
	...
	private function post_list_process_apl_design( $apl_design_slug, $new_design_slug ) {
		...
		if ( isset( $_POST['apl_before'] ) ) {
			$tmp_apl_design_before = wp_kses( filter_input( INPUT_POST, 'apl_before', FILTER_UNSAFE_RAW ), apl_allowed_tags_before() );
		}
		...
		if ( isset( $_POST['apl_content'] ) ) {
			$tmp_apl_design_content = wp_kses( filter_input( INPUT_POST, 'apl_content', FILTER_UNSAFE_RAW ), apl_allowed_tags_content() );
		}
		...
		if ( isset( $_POST['apl_after'] ) ) {
			$tmp_apl_design_after = wp_kses( filter_input( INPUT_POST, 'apl_after', FILTER_UNSAFE_RAW ), apl_allowed_tags_after() );
		}
		...
		if ( isset( $_POST['apl_empty_enable'] ) && isset( $_POST['apl_empty_message'] ) ) {
			$tmp_apl_design_empty = wp_kses( filter_input( INPUT_POST, 'apl_empty_message', FILTER_UNSAFE_RAW ), apl_allowed_tags_before() );
		}
